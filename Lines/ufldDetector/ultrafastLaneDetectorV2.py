...
try :
	...
	from Lines.ufldDetector.core import LaneDetectBase
	from Engine.core import TensorRTEngine
except :
	import sys
...
class UltrafastLaneDetectorV2(LaneDetectBase):
	...
	def _initialize_model(self, model_path : str) -> None:
		...
		if model_path.endswith('.trt') :
			...
			print("[!]Support onnx models will be add in future...")
		if (self.logger) :
		...
		if (len(self.output_names) != 4) :
			raise Exception("[E]Output dims is error, please check model. load %d channels not match 4." % len(self.output_names))
