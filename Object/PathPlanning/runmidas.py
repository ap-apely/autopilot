...
def process(device, model, model_type, image, input_size, target_size, optimize, use_camera):
    ...
    if "openvino" in model_type:
        ...
        prediction = model_midas(sample)[model_midas.output(0)][0]
...
def run(frame, model_midas, transform, net_w, net_h, device, input_path, output_path, model_type="dpt_beit_large_512", optimize=False, side=False, height=None,
    ...
    model_midas, transform, net_w, net_h = load_model(device, model_path, model_type, optimize, height, square)
    '''
    # get input
    ...
    if input_path is not None:
        ...
        for index, image_name in enumerate(image_names):
            ...
            processing_start = time.process_time()

            # compute
            with torch.no_grad():
                prediction = process(device, model_midas, model_type, image, (net_w, net_h), original_image_rgb.shape[1::-1],
                                     optimize, False)
                print(f"processing_time: {time.process_time() - processing_start}")
        ...
        with torch.no_grad():
            ...
            if frame is not None:
                # cv2.imshow("frame in midas", frame)
                original_image_rgb = np.flip(frame, 2)  # in [0, 255] (flip required to get RGB)
                image = transform({"image": original_image_rgb/255})["image"]

                prediction = process(device, model_midas, model_type, image, (net_w, net_h),
                ...
                if time.time()-time_start > 0:
                    ...
                    time_start = time.time()
                    print("-------------------------------")
                print(f"\rMiDaS FPS: {round(fps,2)}", end="\n")
